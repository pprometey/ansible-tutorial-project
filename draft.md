...
```
После выполнения будет запрошен пароль и его подтвердение, после ввода которого откроется редактор vim с помощью которого можно будет отредактировать и сохранить вновь созданный зашифрованный файл.
...
```
Будет запрошен пароль для дешифрования, после чего откроется редактор vim, с помощью которого можно бдует отредактировать зашифрованный файл и сохранить изменения. 
...
После выполнени команды и указания пароля для шифрования файл hi.txt будет перезаписан с уже зашифрованным содержимым. 

Расшифровать файл обратно вручную, можно с помощью команды `decrypt`
...
``
Файл с паролем должен содержать в только одну строку с одинм паролем, и больше ничего.

### Шифрование отдельных строк

Начиная с версии 2.3+ ансибл поддерживает шифровние отдельных строк, и это самый распространенный сценарий использования ansible-vault, когда шифруются только отдельные чувствительные данные (пароли, токены т.д.). Рассмотрим более подробно этот кейс. 

Формат команды для шифрования отдельных переменных такой:
```sh
ansible-vault encrypt_string 'секретное_значение' --name 'имя_переменной'
```
После запуска команды будет запрошен пароль и его подтверждение для шифрования, и будет выведена переменная с зашифрованным значением. 
...
db_password: !vault |
          ...
          6435
```
Соотетственно, мы можем сразу перенаправить этот вывод, чтобы добавить значение в файл с переменными, например:
```
{ echo ""; ansible-vault encrypt_string 'SuperSecret123' --name 'db_password'; } >> group_vars/all.yml
```
Проверим содержимое файла с переменными `cat group_vars/all.yml`
...
db_password: !vault |
          ...
          6638
```
Если необходимо использовать несколько секретных переменных, их необходимо шифровать одинм и тем же паролем. Ансибл позволяет использовать несколько разных паролей, но этот вариант использования выходит за пределы этого базового курса, и мы не будем его здесь рассматривать.

Напишем простой плейбук `ansiblea_vault.yml`, который будет выводить нам расшифрованное значение секретной переменной:
...
Ansible может использоваться в обоих подходах, но особенно хорошо себя показывает в "pet"-подходе. Если же вы строите инфраструктуру по принципам "cattle", рассмотрите использование Terraform, Kubernetes и другие инструменты, ориентированные на декларативное описание состояний.